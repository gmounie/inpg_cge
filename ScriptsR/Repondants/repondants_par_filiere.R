...
levels(a$FSN) = c(1:12)
p = ggplot(a, aes(x=FSN,fill=Filiere)) + geom_bar(aes(show_guide=FALSE))+coord_flip()+ geom_text(label="FILIÈRE FINANCE décalé de 1 an (sous-représenté)!!!", color="red", x=3, y=100) + xlab("") + ylab("")
p
ggsave("../../Output/ensimag_2016_repondants_filiere.png", width=1.2*par("din")[1])
