...
library(ggplot2)
p = ggplot(data = repondants_CGE13, aes(x=as.factor(promo), weight=100*repondants/taillepromo)) + geom_bar()
p +  opts(title="Pourcentage de répondants, début 2O13, promo à 6 et 18 mois") + xlab("Promo Ensimag") + ylab("Pourcentage")  + facet_grid(. ~ annee) + coord_flip()
